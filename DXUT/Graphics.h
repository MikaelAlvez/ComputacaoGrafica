...
private:
    // configurações
    uint backBufferCount;                                   // número de buffers no swap chain
...
public:
    ...
    void Allocate(uint type,
        uint sizeInBytes,
        ID3D12Resource** resource);                         // aloca memória da CPU para recurso
    ...
    uint Antialiasing();                                    // retorna número de amostra por pixel
