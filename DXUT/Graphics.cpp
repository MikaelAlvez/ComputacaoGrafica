...
void Graphics::Initialize(Window* window)
{
	...
	ThrowIfFailed(factory->CreateSwapChainForHwnd(
		...
		window->Id(),							// identificador de janela
		&swapChainDesc,							// descrição de swap chain
		nullptr,								// swap chain para tela cheia
		nullptr,								// restringir tela de saida
	...
	// descreve e cria a heap para o descritor tipo render target (RT)
	...
	// pega um handle para o inicio do heap
	D3D12_CPU_DESCRIPTOR_HANDLE rtHandle = renderTargetHeap->GetCPUDescriptorHandleForHeapStart();

	// valor a incrementar para acessar o próximo dentro da Heap
	...
	// ---------------------------------------------------
	// Depth/Stencil Views ( e heaps associadas )
	// ---------------------------------------------------
	...
	// propriedade da Heap do buffer Depth/Stencil
	...
	// descreve os valores para a limpeza do Depth/Stencil buffer
	...
	// pega um handle para o inicio da heap
	D3D12_CPU_DESCRIPTOR_HANDLE dsHandle = depthStencilHeap->GetCPUDescriptorHandleForHeapStart();

	// dria um descritor (view) de Depth/Stencil para o mip nível 0
	...
	// submete comandos para transição do depth/stencil bufffer 
	SubmitCommands();

	// ---------------------------------------------------
	// Viewport e Retângulo de corte
	// ---------------------------------------------------
...
{
	// reutiliza a memória acessada com a lista de comandos
	// a lista de comandos deve ter terminado de executar na CPU
	commandListAlloc->Reset();

	// uma lista de comandos pode ser reutilizada depois de 
	// adicionada à fila de comandos na GPU (via ExecuteCommandList))
	...
	//especifica quais bufffers serão utilizados na renderização
...
void Graphics::Copy(const void* vertice, uint sizeInBytes, ID3DBlob* bufferCPU)
{
	CopyMemory(bufferCPU->GetBufferPointer(), vertice, sizeInBytes);
}
...
void Graphics::Copy(const void* vertices, uint sizeInBytes, ID3D12Resource* bufferUpload, ID3D12Resource* bufferGPU)
{
	...
	bufferUpload->Map(0, &range, (void**)(&pData));
